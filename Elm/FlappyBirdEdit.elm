...
gravity (model,cmd) = ({model | momentium = model.momentium + 0.15},Cmd.none)
...
wrapStuff (model, cmd) = 
    ...
    then ({ model | dangerOne = {x = 1200, y = (0 + model.randomNumb*20)}}, Cmd.none) 
    else (model, Cmd.none)
...
view model = let
    ...
    then
         ...
         , p [] [Html.text "Press F5 to try again!"]
           ...
           div [Attr.align "center"] 
           [
           div [] [Html.text ("Score: " ++ (toString model.myScore))]
           ,svg [width "760", height "567"]  
          ([image [x "0", y "0", width "100%", height "100%", Svg.Attributes.xlinkHref "./background.png"][]]
              ++ [image [x posX, y posY, width "140", height "80", Svg.Attributes.xlinkHref "./bird.png"][]]
