...
#define led1 RB6 // Defines leds so referring to the data sheet is not required
#define led2 RB5
#define led3 RB4
#define led4 RB7
#define led5 RC7
...
void AltDisplayTemp(int);
void ClearLed();

void main(void)
{ 
	init_hardware();           //run procedure init_hardware()
	//int nMode=0;			   // For button code to switch between modes. Removed in this version.
	while(1) 				   //ToDo use button input as delay
	{						   // Main loop
	int nRaw=0;
	nRaw = GetTempRaw();
	//GetDebug(nRaw);
	...
	//if(nMode==1)
	//{
	AltDisplayTemp(nTemp);
	_delay(2000000);
	//}
	//nMode = CheckButton(nMode);
	ClearLed();
	}
}
/*
int CheckButton(int nMode)
{
for(int iii=0; iii<DELAYTIME; iii++)
	{
		if (RC0==1)
		{
			if(nMode==0)
...
*/
void AltDisplayTemp(int nTemp)
{
	if(nTemp>35) // Lights up all the leds to the maximum for the given temp
	{
		led9=1;
		led1=1;
		led2=1;
		...
		led8=1;
		led9=1;
	_delay(1000000); // About one second
		return;
	}
	if(nTemp>30)
	{
...
{
	if(nTemp>35) 		// Alt way	of displaying test
	{					// It lights up both led's to show numbers between a 5 step range
		led9=1;
		_delay(1000000);
		return;
	}
...
int GetTempRaw()
{
	int nRaw=0; // Gets raw numbers based on the temp
	GO_DONE=1;	// Higher numbers indicate a lower temp
	while(GO_DONE==1)
	{
...
void init_hardware(void)   //set it and forget it
{
	...
	TRISA = 0b00010000;	 //Sets the temp sensor as an input
	TRISB = 0b00000000;
	TRISC = 0b00000000;
	...
	PORTB = 0b00000000;
	PORTC = 0b00000000;	
	ADCON0 =0b00001101; //Clears all the leds so we start from a blank slate
	RC7=0;
	RB7=0;
	RC6=0;
	RC3=0;
